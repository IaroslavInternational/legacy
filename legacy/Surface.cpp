...
Surface::Surface( unsigned int width,unsigned int height )
{
	...
	if( FAILED( hr ) )
	{
		throw Surface::Exception( __LINE__,__FILE__,"Ошибка при инициализации скретч-изображения",hr );
	}
}
...
Surface Surface::FromFile( const std::string& name )
{
	...
	if( FAILED( hr ) )
	{
		throw Surface::Exception( __LINE__,__FILE__,name,"Ошибка загрузки изображения",hr );
	}
	...
	if( scratch.GetImage( 0,0,0 )->format != format )
	{
		...
		if( FAILED( hr ) )
		{
			throw Surface::Exception( __LINE__,__FILE__,name,"Ошибка конвертации изображения",hr );
		}
...
void Surface::Save( const std::string& filename ) const
{
	const auto GetCodecID = []( const std::string& filename ) {
		...
		throw Exception( __LINE__,__FILE__,filename,"Неподдерживаемый формат изображения" );
	};
	...
	if( FAILED( hr ) )
	{
		throw Surface::Exception( __LINE__,__FILE__,filename,"Не удаётся сохранить файл",hr );
	}
}
	...
	note( "[Запись] " + note )
{
	if( hr )
	{
		note = "[Строка ошибки] " + Window::Exception::TranslateErrorCode( *hr ) + "\n" + note;
	}
}
...
const char* Surface::Exception::what() const noexcept
{
	...
	oss << EngineException::what() << std::endl << GetNote();
	whatBuffer = oss.str();
